...
public class SyntaxAnalyzer {
    ...
    private List<Token>tokenStack = new ArrayList<>();
    ...
    public void run() {
        ...
        for(int index = 0; index < tokens.size(); index++){
            ...
            // 找到当前状态的动作
            Action action = states.get(states.size() - 1).getAction(currentToken.getKind());
            // 执行动作
            switch (action.getKind()){
                case Shift -> {
                    states.add(action.getStatus());
                    callWhenInShift(states.get(states.size() - 1), currentToken);
                }
                case Reduce -> {
                    ...
                    states.add(states.get(states.size() - 1).getGoto(production.head()));
                    index--;
                    callWhenInReduce(states.get(states.size()-1), production);
                }
                case Accept -> {
                    callWhenInAccept(states.get(states.size() - 1));
                }
                case Error -> {
