...
public class SymbolTable {
    List<SymbolTableEntry>table = new ArrayList<>();
    /**
     * 获取符号表中已有的条目
     *
     * @param text 符号的文本表示
    ...
    public SymbolTableEntry get(String text) {
        for (SymbolTableEntry symbolTableEntry : table) {
            if (symbolTableEntry.getText().equals(text)) {
    ...
    public SymbolTableEntry add(String text) {
        if(has(text))throw new RuntimeException("Symbol already exists in table");
    ...
    private Map<String, SymbolTableEntry> getAllEntries() {
        final var entries = new HashMap<String, SymbolTableEntry>();
        for (final var entry : table) {
            entries.put(entry.getText(), entry);
        }
        return entries;
    }
